...
# Институционални думи
INSTITUTION_WORDS = [
    "галерия", "библиотека", "музей", "център", "фондация",
    "сдружение", "клуб", "академия", "университет", "читалище",
    "театър", "кино", "музика", "култура", "новини", "събития"
]

# Жанрови думи
GENRE_WORDS = [
    "литература", "изкуства", "поезия", "проза", "есе",
    "роман", "критика", "хайку", "фантастика"
]

# Институционални абревиатури (разширени)
INSTITUTION_PREFIXES = [
    "РБ", "НБ", "СБХ", "НХГ", "НБУ", "БНР", "БНТ", "БТА",
    "НДК", "ОКИ", "ДКТ", "ДТ", "ОУ", "СУ", "ПГ", "НУ", "НЧ"
]
...
def looks_like_non_author(name, slug):
    ...
    # 4) Институционални абревиатури
    for prefix in INSTITUTION_PREFIXES:
        if name.startswith(prefix + " "):
            return True

    # 5) slug съдържа цифри
    if re.search(r"\d", slug_l):
        return True

    # 6) slug съдържа институционална дума
    for w in INSTITUTION_WORDS + GENRE_WORDS:
        if w in slug_l:
            return True
...
def looks_like_real_person(name, slug):
    ...
    # 1) Име с две или повече думи
    if len(parts) >= 2:
        return True

    # 2) Еднословно име, започващо с главна буква
    if len(parts) == 1 and name[0].isupper():
        return True

    # 3) slug от вида ime-familia или firstname-lastname
    slug_parts = slug.split("-")
    if 1 <= len(slug_parts) <= 4:
...
def main():
    ...
    # Събиране на текст за ключови думи
    for post in posts:
        combined = clean_text(
            " ".join([
    ...
    for tag in tags:
        ...
        # Логика: НЕ-автор има приоритет над автор
        if type_key == "author":
