...
def detect_author_posts(tag_id, posts):
    """Връща True, ако тагът е свързан с публикации, съдържащи творби."""
...
def detect_person_posts(tag_id, posts):
    """Връща True, ако тагът е свързан с публикации за личност, но без творби."""
    for post in posts:
        if tag_id in post.get("tags", []):
            return True
    return False

def suggest_type(tag, posts):
    name = tag["name"]
    slug = tag["slug"]
...
def main():
    ...
    for tag in tags:
        tag_id = tag["id"]
        suggested_type, action = suggest_type(tag, posts)
